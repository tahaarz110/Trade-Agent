...
## یادداشت‌های مقیاس‌پذیری (بررسی عمیق پیش از فاز ۶)

طبق الزام صریح پرامپت مادر («Must handle hundreds of thousands of trades»)،
پیش از شروع فاز ۶ یک بررسی عمیق مقیاس‌پذیری روی فاز ۰ تا ۵.۵ انجام شد:

- **ایندکس‌ها:** همه ستون‌های پرکاربرد در فیلتر/جوین (`trade_field_values.field_id`
  با ایندکس GIN اضافه روی `value_json`، `attachments.trade_id`،
  `checklist_items.template_id`، `field_options.field_id`،
  `import_batches.account_id`، `mistake_costs.mistake_tag_id`) ایندکس‌گذاری شدند.
- **N+1 رفع‌شده:** `TradeService.get_detail` با `joinedload` مقادیر فیلد پویا
  را در یک کوئری واحد می‌خواند (قبلاً به ازای هر فیلد یک کوئری جدا می‌زد؛
  با اندازه‌گیری واقعی SQL تأیید شد).
- **CASCADE خطرناک:** دو مورد کشف و به RESTRICT اصلاح شد: `trades.account_id`
  و `import_batches.account_id` — حذف حساب دیگر هرگز معاملات/تاریخچه ایمپورت
  را بی‌صدا پاک نمی‌کند.
- **صفحه‌بندی فرانت‌اند:** `fetchAccounts`/`fetchSymbols` با `page_size` ثابت
  می‌توانستند بالای ۲۰۰ رکورد (مثلاً پس از ایمپورت گسترده نمادهای MT5 در
  فاز ۶) ناقص بمانند؛ تابع `fetchAllPages` برای این دو جدول مرجع (نه
  معاملات) اضافه شد.
- **فیلتر `needs_review`:** به endpoint فهرست معاملات اضافه شد (پیش‌نیاز
  صف بازبینی فاز ۶).

### محدودیت شناخته‌شده (نه باگ)
صفحه‌بندی معاملات از نوع OFFSET/LIMIT است (ساده، سازگار با پرش به هر
صفحه، مناسب برای پیمایش معمول). در عمق بسیار زیاد (مثلاً صفحه ۵۰۰۰ از
۵۰۰هزار ردیف بدون فیلتر) کندتر از pagination نوع keyset/cursor می‌شود.
این یک trade-off آگاهانه است، نه نقص؛ در صورت مشاهده کندی واقعی در
استفاده عملی، می‌توان در فاز بعدی به cursor pagination مهاجرت کرد.
